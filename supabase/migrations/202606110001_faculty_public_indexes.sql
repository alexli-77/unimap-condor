...
create index if not exists intake_records_faculty_verified_idx
  on public.intake_extracted_records (record_type, review_status, updated_at desc)
  where record_type = 'faculty_member' and review_status = 'verified';

create index if not exists intake_records_faculty_name_trgm_idx
  on public.intake_extracted_records
  using gin ((extracted_json->>'name') gin_trgm_ops)
  where record_type = 'faculty_member' and review_status = 'verified';

create index if not exists intake_records_faculty_department_idx
  on public.intake_extracted_records ((extracted_json->>'department_name'))
  where record_type = 'faculty_member' and review_status = 'verified';

create or replace view public.university_faculty_directory_public as
select
  r.id,
  r.record_key,
  s.institution_name,
...
create or replace view public.university_faculty_department_summary_public as
select
  institution_name,
  faculty_name,
  department_name,
